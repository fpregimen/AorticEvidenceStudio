...
import { approvalStatus, reviewHasIncompleteApprovals } from "@/lib/approval-completeness";
...
export function ReviewListClient({ items, questionId, routeSegment, hasSynthesis }: { items: Array<{ review: ContentReview; source: PublicSourceRecord }>; questionId: string; routeSegment: string; hasSynthesis: boolean }) {
  ...
  return <main className="shell py-12">
    ...
    <div className="mt-6 space-y-4">{items.map(({ review, source }) => {
      const claims = counts(review.extracted_claims), outcomes = counts(review.outcome_data), status = reviewHasIncompleteApprovals(review) ? "Approval incomplete" : review.review_status;
...
function counts(items: ContentReview["extracted_claims"] | ContentReview["outcome_data"]) { const result = { Pending: 0, Approved: 0, "Approval incomplete": 0, "Needs correction": 0, Excluded: 0 }; for (const item of items) result[approvalStatus(item)]++; return result }
