...
Use this workflow whenever the user asks to prepare a source review, including a short request such as `Prepare source review for AES-REG-001.` It applies to Evaluation Question 2 records under `database/content_reviews/`.

## Safety invariants

- Read `AGENTS.md` before acting and preserve all medical-safety and privacy requirements.
- Work on `main`. Start with `git branch --show-current`, `git status --short`, and `npm run review:status -- <SOURCE_ID>`.
- Accept only a source ID registered in `database/source_catalog.csv` with an existing Q02 review record. Never accept a path, filename, URL, or shell fragment as a source ID.
...
2. Run `npm run review:status -- <SOURCE_ID>`. Read the catalog row and the existing Q02 review record.
...
6. Create or update only `database/content_reviews/Q02_<SOURCE_ID>.json`. Preserve existing specialist decisions unless the specialist explicitly changes them through the UI. Do not alter other evidence records.
...
10. Report identity verification, files changed, claims and outcomes with exact locations, limitations, unresolved ambiguities, check results, and the manual review URL: `http://localhost:3000/evaluation/2/review/<SOURCE_ID>`.
...
1. Start the application and run `npm run review:open -- <SOURCE_ID>` to open the local review page. This command passes only the validated source ID to the browser and never exposes a private PDF path.
...
4. With only the requested review JSON modified on `main`, run `npm run review:finalize -- <SOURCE_ID>`. The command performs a fail-closed preflight, runs all required checks, stages only that review JSON, creates `Finalize specialist review for <SOURCE_ID>`, and pushes `main` to `origin`.
