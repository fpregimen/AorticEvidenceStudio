...
const loader=await readFile("lib/authoring/node-pdfjs-loader.ts","utf8"),extraction=await readFile("lib/authoring/pdf-extraction.ts","utf8"),route=await readFile("app/api/authoring/source-files/[sourceFileId]/extract/route.ts","utf8"),workflow=await readFile("lib/authoring/payload-safe-pdf.ts","utf8");
const canvasImport=loader.indexOf('import("@napi-rs/canvas")'),domInstall=loader.indexOf("globals.DOMMatrix"),pdfImport=loader.indexOf('import("pdfjs-dist/legacy/build/pdf.mjs")');
assert.ok(canvasImport>=0&&domInstall>canvasImport&&pdfImport>domInstall,"canvas globals must be installed before importing PDF.js");
