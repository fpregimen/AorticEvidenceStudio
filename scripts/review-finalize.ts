...
import { finalizationPreflight, loadReviewWorkflowContext, parseGitPorcelain, requireExistingReview, requireQuestionAndSourceIds } from "../lib/review-workflow.ts";
...
try {
  ...
  console.log(`Finalization preflight passed for ${questionId} ${sourceId}. Running required checks before staging.`);
  ...
  run("git", ["commit", "-m", `Finalize ${questionId} specialist review for ${sourceId}`], root);
  run("git", ["push", "origin", "main"], root);
  console.log(`Finalized and pushed ${questionId} ${sourceId}.`);
} catch (error) {
