...
import { loadReviewWorkflowContext, requireQuestionAndSourceIds, reviewDecisionCounts } from "../lib/review-workflow.ts";

try {
  const { questionId, sourceId } = requireQuestionAndSourceIds(process.argv.slice(2));
  const root = process.cwd();
  const context = await loadReviewWorkflowContext(root, questionId, sourceId);
  const branch = execFileSync("git", ["branch", "--show-current"], { cwd: root, encoding: "utf8" }).trim();
  const gitStatus = execFileSync("git", ["status", "--porcelain=v1", "--untracked-files=all"], { cwd: root, encoding: "utf8" });
  const counts = context.review ? reviewDecisionCounts(context.review) : { approved: 0, correctionRequired: 0, pending: 0, excluded: 0, total: 0 };
  console.log(`Source review status: ${questionId} ${sourceId}`);
  ...
  console.log(`Review JSON exists: ${context.review ? "Yes" : "No"}`);
  console.log(`Current review status: ${context.review?.review_status ?? "Not started"}`);
