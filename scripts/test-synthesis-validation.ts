...
const api=await readFile(new URL("../lib/synthesis-route-handlers.ts",import.meta.url),"utf8");
...
assert.match(api,/reviewer_name\?\.trim\(\)/);assert.match(api,/validIsoDate\(review\.review_date\)/);assert.match(api,/!review\.confirmed/);assert.match(api,/review\.decision\s*===\s*"Return for correction"\s*&&\s*!review\.reviewer_note\.trim\(\)/);
...
assert.match(client,/Evidence review warnings/);assert.match(client,/href=\{`\/evaluation\/\$\{routeSegment\}\/review\/\$\{row\.sourceId\}`\}/);
